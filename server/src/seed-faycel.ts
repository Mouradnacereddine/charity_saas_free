...
async function main() {
  ...
  // Articles
  await Promise.all([
    prisma.article.create({ data: { associationId: aid, reference: ref('ART'), name: 'Lit medicalise', nameAr: 'سرير طبي', categoryId: (await prisma.articleCategory.findFirst({ where: { associationId: aid, nameAr: 'طبي' } }))!.id, quantity: 5, availableQuantity: 3, status: 'disponible', storageLocationId: (await prisma.storageLocation.findFirst({ where: { associationId: aid, nameAr: 'المستودع الطبي' } }))!.id, isPermanent: false, createdAt: now, updatedAt: now } }),
    prisma.article.create({ data: { associationId: aid, reference: ref('ART'), name: 'Colis alimentaire', nameAr: 'سلة غذائية', categoryId: (await prisma.articleCategory.findFirst({ where: { associationId: aid, nameAr: 'غذائي' } }))!.id, quantity: 80, availableQuantity: 80, status: 'disponible', isPermanent: true, createdAt: now, updatedAt: now } }),
    prisma.article.create({ data: { associationId: aid, reference: ref('ART'), name: 'Cartable scolaire', nameAr: 'حقيبة مدرسية', categoryId: (await prisma.articleCategory.findFirst({ where: { associationId: aid, nameAr: 'مدرسي' } }))!.id, quantity: 30, availableQuantity: 30, status: 'disponible', isPermanent: false, createdAt: now, updatedAt: now } }),
  ]);
