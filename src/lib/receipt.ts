...
export function printReceipt(
  ...
  if (!w) return
  w.document.write(`
<!DOCTYPE html><html dir="${dir}" lang="${lang}"><head><meta charset="UTF-8"><title>${title}</title>
<style>
  @page { size: A4; margin: 8mm 6mm; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', Tahoma, sans-serif; direction: ${dir}; font-size: 10px; background: #eee; padding: 5mm 0 0 0; }
  ...
  .hdr .sub { float: ${dir === 'rtl' ? 'left' : 'right'}; font-size: 7px; color: #999; }
...
export function printBeneficiaryCard(params: {
  ...
  const w = window.open('', '_blank')
  if (!w) return
  const html = `<!DOCTYPE html><html dir="${d}" lang="${ln}"><head><meta charset="UTF-8"><title>${l.title}</title>
  <style>
    @page { size: A4 portrait; margin: 0; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Segoe UI', Tahoma, sans-serif; direction: ${d}; font-size: 11px; color: #1a1a1a; background: #fff; padding: 15mm; }
    @media print { body { padding: 15mm; } }
    .hdr { border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin-bottom: 8px; }
    .hdr h1 { font-size: 16px; color: #2563eb; margin: 0; display: inline; }
    .hdr .sub { float: ${d === 'rtl' ? 'left' : 'right'}; font-size: 10px; color: #999; }
    .section { margin-bottom: 10px; }
    .section-title { font-size: 12px; font-weight: 700; color: #1e40af; margin: 0 0 4px; padding: 3px 8px; background: #eff6ff; border-${d === 'rtl' ? 'right' : 'left'}: 3px solid #2563eb; }
    .info { display: flex; flex-wrap: wrap; gap: 0 10px; }
    .col { width: 48%; }
    .row { margin: 0.3mm 0; line-height: 1.45; }
    .row .lbl { display: block; font-size: 8px; color: #aaa; }
  ...
  <div class="section">
    ...
    <div class="info">
      ...
      <div class="col">
        ...
        ${params.situation ? `<div class="border-row"><span class="lbl">${l.status}</span><span class="val">${params.situation}</span></div>` : ''}
      </div>
    </div>
  </div>
  ${params.childrenHtml}
  <div class="sign">
    <div><span class="label">${l.beneficiarySignature}</span><div class="line"></div></div>
    <div><span class="label">${l.stampSignature}</span><div class="line"></div></div>
  </div>
  <button class="no-print" onclick="window.print()">${l.print}</button>
</body></html>`
...
export function printAnalyticsReport(params: {
  ...
  const l = params.labels || {
    totalIncome: 'إجمالي المقبoustات',
    totalExpenses: 'إجمالي المدفوعات',
    netFinancial: 'الصافي المالي للمرحلة',
    expenseToIncome: 'معدل المصاريف إلى المداخيل',
  ...
  const REPORT_CSS = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .section-title { font-size: 12px; font-weight: 700; color: #1e40af; margin: 0 0 6px; padding: 4px 8px; background: #eff6ff; border-${d === 'rtl' ? 'right' : 'left'}: 3px solid #2563eb; }
  ...
  <div class="kpi-grid">
    <div class="kpi"><div class="kpi-label">${l.totalIncome}</div><div class="kpi-value" style="color:#10b981">${params.credits}</div></div>
    <div class="kpi"><div class="kpi-label">${l.totalExpenses}</div><div class="kpi-value" style="color:#ef4444">${params.debits}</div></div>
    <div class="kpi"><div class="kpi-label">${l.netFinancial}</div><div class="kpi-value" style="${(params.balance.startsWith('-')) ? 'color:#ef4444' : 'color:#2563eb'}">${params.balance}</div></div>
    <div class="kpi"><div class="kpi-label">${l.expenseToIncome}</div><div class="kpi-value">${params.ratio}</div></div>
  </div>
  <div class="section">
    ${params.bodyRows}
  </div>
 ...
 * advanced search as one card per beneficiary (own info grid + dedicated
 * children table with its own headers), so beneficiary and child data
 * never share the same column headers.
 */
export function printBeneficiariesList(params: {
  ...
  cardsHtml: string;      // one .benef-card per beneficiary, composed by the caller
  labels?: Record<string, string>;
  lang?: string;
  dir?: 'ltr' | 'rtl';
}) {
  ...
  const REPORT_CSS = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .criteria { margin-bottom: 10px; display: flex; flex-wrap: wrap; align-items: center; gap: 4px; }
    .criteria .criteria-label { font-size: 9.5px; font-weight: 700; color: #374151; margin-inline-end: 3px; }
    ...
    /* ===== Beneficiary card ===== */
    .benef-card { border: 1px solid #d1d5db; border-radius: 6px; margin-bottom: 8px; overflow: hidden; page-break-inside: avoid; }
    .benef-header { background: #eff6ff; padding: 5px 10px; display: flex; align-items: center; gap: 8px; border-bottom: 1px solid #bfdbfe; }
    .benef-header .num { background: #2563eb; color: #fff; border-radius: 50%; min-width: 16px; height: 16px; font-size: 9px; font-weight: 700; display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .benef-header .name { font-weight: 700; color: #1e3a8a; font-size: 11.5px; }
    .benef-header .ref { color: #6b7280; font-size: 9px; }
    .benef-header .attr { margin-inline-start: auto; background: #2563eb; color: #fff; border-radius: 10px; padding: 1px 8px; font-size: 9px; font-weight: 600; }
    .benef-info { display: flex; flex-wrap: wrap; }
    .benef-info .cell { width: 25%; padding: 4px 10px; border-bottom: 0.5px solid #f1f5f9; }
    .benef-info .cell .lbl { font-size: 7.5px; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.3px; display: block; }
    .benef-info .cell .val { font-size: 10px; color: #111827; font-weight: 600; }

    /* ===== Children table (own headers) ===== */
    .children-title { font-size: 9.5px; font-weight: 700; color: #1e40af; padding: 4px 10px; background: #f8fafc; border-top: 0.5px solid #e5e7eb; }
    table.children { width: 100%; border-collapse: collapse; }
    table.children th { background: #f0f4ff; color: #1e40af; font-size: 8.5px; font-weight: 700; padding: 3px 10px; text-align: ${d === 'rtl' ? 'right' : 'left'}; border-top: 0.5px solid #dbeafe; border-bottom: 0.5px solid #dbeafe; }
    table.children td { font-size: 9px; padding: 3px 10px; border-top: 0.5px solid #f1f5f9; color: #1f2937; }
    table.children .c-num { width: 7mm; color: #6b7280; }
    table.children .c-name { font-weight: 600; color: #111827; }
  ...
  <div class="criteria"><span class="criteria-label">${params.criteriaLabel}</span>${params.criteriaHtml}<span class="chip count">${params.countLabel}</span></div>
  ${params.cardsHtml}
