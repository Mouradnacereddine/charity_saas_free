                ...
                <tbody>
                  {stats.recentTransactions.map((tx: any) => (
                    <tr key={tx.id} className="border-b border-border hover:bg-muted transition-colors cursor-pointer" onClick={() => setDetailTx(tx)}>
                      ...
                      <td className="py-3 px-4 text-foreground hidden sm:table-cell">{tx.descriptionAr}</td>
        ...
        {detailTx && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-muted rounded-lg p-4">
              ...
              {detailTx.descriptionAr && <div className="sm:col-span-2"><p className="text-xs text-muted-foreground">{t('common.description')}</p><p className="font-medium text-foreground">{detailTx.descriptionAr}</p></div>}
